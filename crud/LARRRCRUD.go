...
func (crud LARRRCRUD) Create(rrrIn interface{}) (interface{}, error) {
	...
	existing := 0
	reader := tx.Model(&ladm.LARRR{}).Where("rid = ?::\"Oid\" AND endlifespanversion IS NULL", rrr.RID).
		Count(&existing)
	if reader.Error != nil{
		tx.Rollback()
...
func (crud LARRRCRUD) Update(rrrIn interface{}) (interface{}, error) {
	...
	rrr.BeginLifespanVersion = currentTime
	rrr.EndLifespanVersion = nil
	rrr.PartyID = oldRrr.PartyID
	rrr.PartyBeginLifespanVersion = oldRrr.PartyBeginLifespanVersion
	rrr.UnitID = oldRrr.UnitID
	rrr.UnitBeginLifespanVersion = oldRrr.UnitBeginLifespanVersion
	writer = tx.Set("gorm:save_associations", false).Create(&rrr)
	if writer.Error != nil {
		tx.Rollback()
		return nil, writer.Error
